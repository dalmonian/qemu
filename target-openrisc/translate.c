...
static void dec_calc(DisasContext *dc, uint32_t insn)
{
    ...
    case 0x0000:
        ...
        case 0x00:    /* l.add */
            ...
            {
                gen_helper_adder(cpu_R[rd], cpu_env, cpu_R[ra], cpu_R[rb], 0);
            }
            break;
        default:
            gen_illegal_exception(dc);
    ...
    case 0x0001:    /* l.addc */
        ...
        case 0x00:
            ...
            {
                TCGv_i32 tcy = tcg_temp_local_new_i32();
                tcg_gen_andi_i32(tcy, cpu_sr, SR_CY);
                tcg_gen_shri_i32(tcy, tcy, 9);
                gen_helper_adder(cpu_R[rd], cpu_env, cpu_R[ra], cpu_R[rb], tcy);
    ...
    case 0x0009:
        ...
        case 0x03:    /* l.div */
            LOG_DIS("l.div r%d, r%d, r%d\n", rd, ra, rb);
            {
                ...
                TCGv_i32 aecr_dbze = tcg_temp_local_new_i32();
                TCGv_i32 excp = tcg_temp_local_new_i32();
                if (rb == 0 || cpu_R[rb] == 0) {
                    tcg_gen_ori_tl(cpu_sr, cpu_sr, SR_OV);
                    tcg_gen_andi_tl(sr_ove, cpu_sr, SR_OVE);
                    tcg_gen_andi_tl(aecr_dbze, cpu_aecr, AECR_DBZE);
                    tcg_gen_setcondi_tl(TCG_COND_EQ, sr_ove, sr_ove, 0x0);
                    tcg_gen_setcondi_tl(TCG_COND_EQ, aecr_dbze, aecr_dbze, 0x0);
                    tcg_gen_and_tl(excp, sr_ove, aecr_dbze);
                    tcg_gen_brcondi_tl(TCG_COND_NE, excp, 0x0, lab);
                    tcg_gen_ori_tl(cpu_aesr, cpu_aesr, AESR_DBZE);
...
/*#ifdef TARGET_OPENRISC64
    ...
    case 0x27:    /* l.addi */
        ...
        {
            TCGv_i32 t = tcg_temp_local_new_i32();
            tcg_gen_movi_tl(t, I16);
            if (I16 == 0) {
                tcg_gen_mov_tl(cpu_R[rd], cpu_R[ra]);
            } else {
                gen_helper_adder(cpu_R[rd], cpu_env, cpu_R[ra], t, 0);
            }
            tcg_temp_free_i32(t);
        }
        break;
    ...
    case 0x28:    /* l.addic */
        LOG_DIS("l.addic r%d, r%d, %d\n", rd, ra, I16);
        {
            ...
            tcg_gen_andi_i32(tcy, cpu_sr, SR_CY);
            tcg_gen_shri_i32(tcy, tcy, 9);
            gen_helper_adder(cpu_R[rd], cpu_env, cpu_R[ra], t, tcy);
            tcg_temp_free_i32(tcy);
            tcg_temp_free_i32(t);
        }
        break;
